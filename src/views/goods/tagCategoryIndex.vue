    ...
    >
      ...
      >
        <template slot-scope="{ row }">
          <el-tag v-for="(item, index) in row.tags" :key="index" effect="plain" style="margin-left: 5px">{{ item.name }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        prop="sort"
...
<script>
import { dataList, deleteData, sortCategory } from '@/api/tagCategory'
import { getToken, DominKey } from '@/utils/auth'
import TagCategoryAddOrUpdate from './components/TagCategoryAddOrUpdate'
import Pagination from '@/components/Pagination/index'

export default {
  name: 'Category',
  components: { Pagination, TagCategoryAddOrUpdate },
  data() {
    return {
